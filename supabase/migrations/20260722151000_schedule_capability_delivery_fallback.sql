-- Keep simple schedule-capability questions responsive even when the large X
-- turn processor cannot start. This lane is deliberately narrow: it never
-- handles concrete commands, addresses, amounts, confirmations, or funds.
...
begin
  ...
  loop
    insert into public.twitter_replies (
      ...
      v_tweet.author_twitter_id,
      'schedule_capability_fallback',
    ...
    set status = 'completed',
        processed_at = now(),
        error = null
    where tweet_id = v_tweet.tweet_id
      and status = 'pending';
...
revoke all on function public.deliver_pending_schedule_capability_replies(integer)
  from public, anon, authenticated;
grant execute on function public.deliver_pending_schedule_capability_replies(integer)
