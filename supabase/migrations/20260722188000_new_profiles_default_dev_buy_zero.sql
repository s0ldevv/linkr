-- Every newly created profile must start with a zero default dev buy,
-- regardless of how the user arrives (auth login, X mention provisioning,
-- transfer-recipient provisioning). The columns already carry a DB-level
-- default of 0; this makes the auth-signup trigger explicit so the guarantee
-- survives any future change to the column defaults. The on-conflict branch
-- intentionally never touches these columns: an existing user's chosen
-- default is preserved.

alter table public.profiles
  alter column default_dev_buy_sol set default 0,
  alter column default_dev_buy_eth set default 0;
