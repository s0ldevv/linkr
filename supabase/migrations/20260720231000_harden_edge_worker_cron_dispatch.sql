-- Keep the dispatcher private to pg_cron and limit its internal-key authority
-- to the two worker routes intentionally scheduled by this subsystem.
...
begin
  if p_function_name not in ('cron-process-launches', 'cron-process-scheduled-actions') then
