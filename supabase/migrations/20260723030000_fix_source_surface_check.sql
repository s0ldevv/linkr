-- Update finalize_linkr_coin_launch_v1 to check launch record's source_surface
-- This is a minimal, safe change that fixes the reply issue
-- Migration: 20260723030000_fix_source_surface_check.sql

-- First, verify the current function exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_proc WHERE proname = 'finalize_linkr_coin_launch_v1'
  ) THEN
    RAISE EXCEPTION 'finalize_linkr_coin_launch_v1 does not exist';
  END IF;
END $$;

-- Update the function to check both work_item and launch source_surface
-- Note: coin_launches.source_surface is already populated by worker-media-capture
...
begin
  ...
  -- FIX: Check both work_item and launch source_surface for X replies
  if v_item.source_surface = 'x' OR v_launch.source_surface = 'x' then
...
-- Grant permissions (preserving existing grants)
