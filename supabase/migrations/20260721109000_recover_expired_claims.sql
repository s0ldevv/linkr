-- A worker can terminate after atomically marking a row leased but before
-- completion. When PGMQ visibility and the capacity/resource leases expire,
-- that same pointer is exactly one state version behind and is a valid
-- redelivery, not a stale duplicate.
...
begin
  ...
  loop
    ...
    v_is_expired_redelivery := v_item.state = 'leased'
      and v_item.state_version = v_message_state_version + 1;
