...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const quote = isPumpSwap
      ? await quotePumpAddLiquidity(admin, ctx.userId, ctx.body)
      : await quoteAddLiquidity(admin, ctx.userId, ctx.body);
    if (ctx.body.dry_run === true) {
      await recordAgentRequest(admin, ctx, req, 200);
      return agentJsonResponse({ dry_run: true, quote });
    }
    const idempotencyKey = `agent-liquidity-add:${ctx.apiKeyId}:${ctx.idempotencyKey}`;
