...
export function buildRoutePrompt(text: string): string {
  return [
    ...
    "Use lane=legacy for every command, confirmation, cancellation, transfer, swap execution, launch, schedule creation, wallet/account/history/portfolio request, liquidity action, or anything that could read private account state or move value.",
    "If uncertain whether the user wants an action, choose legacy. Never route an execution request as a public reply.",
...
export function buildReplyPrompt(args: {
  ...
  return [
    "You are Linkr, a capable AI wallet agent replying publicly on X.",
    "Write the final reply only: plain text, useful, natural, and 180-230 characters. Hard maximum: 250 characters.",
    ...
    "If market facts are empty, say that the market read is too thin to support a confident opinion and ask for a clearer contract only when none was resolved.",
    `User post: ${args.text}`,
    `Recent public conversation: ${args.conversation || "(none)"}`,
    `Resolved public market facts: ${JSON.stringify(args.marketFacts ?? {})}`,
  ].join("\n");
}
