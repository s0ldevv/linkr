...
export async function prepareXAirdropXFlow(args: {
  ...
  if (intent.clarification || !intent.token || !intent.amount) {
    return clarification(
      ...
      intent.clarification ??
        "Which Linkr token and total amount should I prepare for the holder airdrop?",
    );
  }
  ...
  try {
    ...
    if (/airdrop_amount/.test(code)) {
      return clarification(
        ...
        "What exact total token amount should I prepare?",
      );
    }
    throw error;
  }
  ...
  const persistedSummary = persisted.data ?? {};
  return {
    replyKind: "airdrop_confirmation_required",
    replyText: `Prepared an immutable snapshot for ${
      String(persistedSummary.recipient_count ?? prepared.allocations.length)
    } eligible holders. Total allocated: ${
      String(persistedSummary.allocated_raw ?? prepared.allocatedRaw.toString())
    } raw units; retained dust: ${
      String(persistedSummary.dust_raw ?? prepared.dustRaw.toString())
    }. Reply to confirm the holder airdrop.`,
    state: "waiting_user_confirmation",
    resultRef: `holder-airdrop:${persisted.data?.airdrop_id ?? "prepared"}`,
  };
}
...
> {
  const result = await admin.from("coin_launches").select(
  ...
  const matches = (result.data ?? []).filter((row: any) => {
    ...
    if (!mint || (row.token_address && String(row.token_address) !== mint)) {
      return false;
    }
    return [mint, row.symbol, row.name].some((value) =>
      String(value ?? "").trim().replace(/^\$/, "").toLowerCase() === needle
    );
  });
